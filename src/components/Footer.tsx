...
const Footer = () => {
  ...
  const socialLinks = [{
    icon: Instagram,
    url: "https://www.instagram.com/trident_jewelles?igsh=MTQ5ZG81MmV1aWtoMg==",
    label: "Instagram",
    image: null
  }, {
    icon: Facebook,
    ...
    icon: Mail,
    url: "mailto:contact@tridentjewellery.com",
    label: "Email",
    image: null
  }, {
    icon: null,
  ...
  return <footer className="relative overflow-hidden bg-background border-t border-border/20">
      ...
      <div className="relative container mx-auto px-4 py-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-12">
          ...
          <div className="mb-0 mt-[25px] ml-[50px]">
            <h3 className="text-xl font-bold text-primary-foreground mb-6 tracking-wider">
              SERVICES
            </h3>
            <ul className="space-y-3">
              {services.map(service => <li key={service.label}>
                  <Link to={service.path} className="text-primary-foreground/80 hover:text-accent transition-colors duration-300 text-sm">
                    {service.label}
                  </Link>
                </li>)}
            </ul>
          ...
          <div className="mt-[25px]">
            ...
            <ul className="space-y-3">
              {usefulLinks.map(link => <li key={link.label}>
                  <Link to={link.path} className="text-primary-foreground/80 hover:text-accent transition-colors duration-300 text-sm">
                    {link.label}
                  </Link>
                </li>)}
            </ul>
